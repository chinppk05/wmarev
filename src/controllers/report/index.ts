...
export const getIncomeFixedCollection = async (request: Request, response: Response) => {
  ...
  let compare3 = contractEnd
  console.log("c1", compare1.startOf("day"))
  console.log("c2", compare2.startOf("day"))
  console.log("c3", compare3.startOf("day"))
  if(compare1.startOf("day")<=compare3.startOf("day")) conditions.push({...conditions[conditions.length-1],isLast:true})
  ...
  conditions.forEach((con,i) => {
    ...
    for (let j = 1; j <= 4; j++) {
      ...
      if(isEnding){
        ...
        if(quarterStart.startOf('day') < contractEnd.startOf('day') && quarterEnd.startOf('day') > contractEnd.startOf('day')) {
          split = [Math.round(contractEnd.diff(quarterStart,'days').days)+1,0]
          rate = [rate[0],0]
        }
        else{
          split = [0,0]
